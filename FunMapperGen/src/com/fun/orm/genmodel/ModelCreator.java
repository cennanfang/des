...
public class ModelCreator {
	...
	private Logger logger  =  Logger.getLogger(ModelCreator.class);
	...
	public ModelCreator(boolean isNeedJavaUtilPackage, boolean isNeedJavaSqlsPackage, String packageOutPath,
			String authorName, String className, List<String> colNames, List<String> colTypes,
		...
		for (int i = 0; i < colNames.size(); i++) {
			...
			if(ci != null) {
				if(ci.getConstraintType().equals(ConstraintType.PRIMARY)) {
					...
					String classType = NameUtils.initcapUpCase(NameUtils.underline2Camel(ci.getRefTable().substring(GenEntityMysql.TABLE_PREFIX.length()), true));
